...
class SUtil
{
	...
	public static function getPath():String
	{
		...
		#if ios
		// TEST/external-assets: prefer reading from Documents (writable, user can drop
		// corruption assets/mods there). Only fall back to bundle-internal if Documents
		// has no external assets/mods.
		var doc:String = LimeSystem.documentsDirectory;
		if (doc != null && doc.length > 0 && !doc.endsWith('/'))
			doc += '/';
		if (FileSystem.exists(doc + 'assets') || FileSystem.exists(doc + 'mods'))
			return doc; // external assets/mods present -> read from Documents
	...
	// Writable location for saves / crash logs (Documents, NOT the read-only bundle).
	...
	// iOS: force-check that external (Documents) assets + mods exist. If missing, write
	// an error to Documents/startup_error.txt and exit (no window.alert on iOS).
	public static function iosExternalCheck():Void
	{
		#if ios
		var doc:String = LimeSystem.documentsDirectory;
		if (doc != null && doc.length > 0 && !doc.endsWith('/'))
		...
		if (!FileSystem.exists(doc + 'assets')) missing.push('assets');
		if (!FileSystem.exists(doc + 'mods'))   missing.push('mods');

		if (missing.length > 0)
		{
			var msg:String = 'MISSING external folder(s) in Documents: ' + missing.join(', ') + '\n' +
				'Expected at: ' + doc + '\n' +
				'Please put your corruption assets/ and mods/ folders here (via Files app / iTunes file sharing).';
			try {
				var f = sys.io.File.write(doc + 'startup_error.txt', false);
				f.writeString(msg + '\n');
				f.close();
			} catch (e:Dynamic) {}
			Sys.println(msg);
	...
	{
		if (!FileSystem.exists(SUtil.getSavePath() + 'saves'))
			FileSystem.createDirectory(SUtil.getSavePath() + 'saves');

		File.saveContent(SUtil.getSavePath() + 'saves/' + fileName + fileExtension, fileData);
