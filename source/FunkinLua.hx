...
class FunkinLua {
	...
	function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		// base assets/ tree for the self-contained iOS build, so they are NOT under
		// a mods/ folder and must be resolved via OpenFlAssets instead).
		var fragPath:String = Paths.getPath('shaders/' + name + '.frag', TEXT);
		var vertPath:String = Paths.getPath('shaders/' + name + '.vert', TEXT);
		if (openfl.utils.Assets.exists(fragPath) || openfl.utils.Assets.exists(vertPath))
		{
			var f:String = openfl.utils.Assets.exists(fragPath) ? openfl.utils.Assets.getText(fragPath) : null;
			var v:String = openfl.utils.Assets.exists(vertPath) ? openfl.utils.Assets.getText(vertPath) : null;
			PlayState.instance.runtimeShaders.set(name, [f, v]);
			return true;
		}
...
class HScript
{
	...
	public function new()
	{
		...
		// V-Slice API compatibility shim
		VSliceShim.register(interp);
