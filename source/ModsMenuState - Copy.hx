...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			remove1(Paths.modFolder() + modsList[curSelected][0]);
	...
	static function remove1(path:String) {
		if(FileSystem.isDirectory(path)) {
		  var list = FileSystem.readDirectory(path);
		  for(it in list) {
			remove1(Path.join([path, it]));
		  }
	...
	function installMod() {
		...
		var directoryContents:Array<String> = sys.FileSystem.readDirectory(Paths.modFolder());
	...
	function unzipthing(string:String):Void
	{
		unzip(Paths.modFolder() + string, Paths.modFolder(), "");
		canExit = true;
		if(ClientPrefs.deleteZip)
		remove1(Paths.modFolder() + string);
		MusicBeatState.resetState();
	}
